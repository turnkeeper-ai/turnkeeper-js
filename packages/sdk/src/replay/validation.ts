...
import {
  ...
  type ReplayEvent,
  type ReplayEventValidationFailure,
  type ReplayEventValidationSuccess,
...
function validateEvent(
...
): ReplayEventValidationFailure | ReplayEventValidationSuccess {
  ...
  try {
    ...
    if (utf8Bytes(canonical) > MAX_REPLAY_EVENT_BYTES) errors.push(issue(path, "event_too_large"));
  } catch {
    errors.push(issue(path, "invalid_json_value"));
  }
  if (!checkShape(value, path, EVENT_KEYS, errors, REQUIRED_EVENT_KEYS)) {
  ...
  if (hasOwn(value, "occurred_at")) {
    ...
    else if (enforceRetention && occurredAt !== null && occurredAt < nowMs - retentionMs) {
      errors.push(issue(`${path}.occurred_at`, "timestamp_outside_retention"));
    }
  }
  ...
  if (errors.length === 0) {
    const event = value as unknown as ReplayEvent;
    return {
      contentHash,
      event,
      index,
  ...
  const sourceEventId = isOpaqueReplayId(value.source_event_id)
    ? { source_event_id: value.source_event_id }
    : {};
  return { errors, index, ok: false, ...sourceEventId };
}
...
export function validateReplayBatch(
  ...
  if (typeof enforceRetention !== "boolean") envelopeErrors.push(issue("$", "invalid_retention_option"));
  if (
    enforceRetention &&
...
export const stableCanonicalJSON = stableCanonicalJson;

export function canonicalSHA256(value: unknown): string {
  return createHash("sha256").update(stableCanonicalJson(value), "utf8").digest("hex");
}

export const validateReplayEnvelope = validateReplayBatch;
