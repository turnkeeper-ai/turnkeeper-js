...
if (manifest.dependencies?.["@turnkeeper/sdk"] !== "0.1.0-alpha.1") {
  throw new Error(
...
for (const lifecycle of ["install", "postinstall", "preinstall"]) {
  if (manifest.scripts?.[lifecycle]) {
    throw new Error(
      `CLI package must not declare a ${lifecycle} lifecycle script.`,
    );
  }
}
...
console.log(`CLI package contents verified (${files.length} files).`);
